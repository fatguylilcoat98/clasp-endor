...
All nine previous guards plus the RLS / privacy contract are
...
- The **`unit-tests`** job runs the `node:test` unit suite for the
  ...
  and the memory module under `src/memory/` (`tests/memory/*.test.js`).
...
- The **`integration-tests`** job boots the runtime against a
  ...
  RLS-engagement and the GM-17 memory-governance contracts
  (`tests/integration/*.test.js`). The setup step creates three LOGIN
  roles (`lylo_runtime_login`, `lylo_setup_login`, `lylo_app_login`)
  with the per-role `BYPASSRLS` posture documented in
